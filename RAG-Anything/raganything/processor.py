...
class ProcessorMixin:
    ...
    async def _mark_multimodal_processing_complete(self, doc_id: str):
        ...
        try:
            current_doc_status = await self.lightrag.doc_status.get_by_id(doc_id)
            if current_doc_status:
                # Create a new status dict without problematic parameters
                updated_status = {
                    **current_doc_status,
                    "updated_at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00"),
                }
                # Add multimodal_processed to the status dict only (not as constructor param)
                updated_status["multimodal_processed"] = True
