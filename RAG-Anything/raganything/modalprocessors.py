...
class BaseModalProcessor:
    ...
    def _coerce_schema(self, response_data: dict, default_entity_type: str, fallback_text: str = "") -> dict:
        """Coerce loosely-structured JSON into the required schema.
        Ensures keys: detailed_description, entity_info.{entity_name, entity_type, summary}
        """
        if not isinstance(response_data, dict):
            return {
                "detailed_description": fallback_text or "",
                "entity_info": {
                    "entity_name": f"unknown_{default_entity_type}",
                    "entity_type": default_entity_type,
                    "summary": (fallback_text or "")[:100],
                },
            }

        # Coerce description
        desc = response_data.get("detailed_description")
        if not desc:
            for k in ["description", "analysis", "caption", "content", "text"]:
                if isinstance(response_data.get(k), str) and response_data.get(k).strip():
                    desc = response_data.get(k)
                    break
        if not desc:
            # Try nested common holders
            for k in ["result", "data", "response"]:
                v = response_data.get(k)
                if isinstance(v, dict):
                    for sk in ["detailed_description", "description", "analysis", "caption"]:
        ...
        # Coerce entity_info
        ent = response_data.get("entity_info")
        ...
        if not name:
            for k in ["name", "title", "entity", "label"]:
                if isinstance(ent.get(k), str) and ent.get(k).strip():
                    name = ent.get(k)
                    break
            if not name:
                # look at top-level
                for k in ["entity_name", "name", "title", "label"]:
                    if isinstance(response_data.get(k), str) and response_data.get(k).strip():
                        name = response_data.get(k)
                        break
        if not name:
            name = f"unknown_{default_entity_type}"
        ...
        if not etype:
            for k in ["type", "kind", "category"]:
                if isinstance(ent.get(k), str) and ent.get(k).strip():
                    ...
                    break
            if not etype:
                for k in ["entity_type", "type", "category"]:
                    if isinstance(response_data.get(k), str) and response_data.get(k).strip():
                        etype = response_data.get(k)
                        break
        if not etype:
        ...
        if not summary:
            for k in ["summary", "short_description", "caption", "brief"]:
                if isinstance(ent.get(k), str) and ent.get(k).strip():
                    summary = ent.get(k)
                    break
            if not summary:
                for k in ["summary", "short_description", "caption"]:
                    if isinstance(response_data.get(k), str) and response_data.get(k).strip():
                        summary = response_data.get(k)
                        break
        if not summary:
            summary = (desc or fallback_text or name)[:100]

        return {
            "detailed_description": desc or (fallback_text or ""),
            "entity_info": {
                "entity_name": name,
                "entity_type": etype,
                "summary": summary,
...
class ImageModalProcessor(BaseModalProcessor):
    ...
    def _parse_response(
        ...
        try:
            ...
            # Coerce schema if fields missing
            if not isinstance(response_data, dict) or not response_data.get("detailed_description") or not response_data.get("entity_info"):
                response_data = self._coerce_schema(response_data if isinstance(response_data, dict) else {}, "image", response)
...
class TableModalProcessor(BaseModalProcessor):
    ...
    def _parse_table_response(
        ...
        try:
            response_data = self._robust_json_parse(response)
            if not isinstance(response_data, dict) or not response_data.get("detailed_description") or not response_data.get("entity_info"):
                response_data = self._coerce_schema(response_data if isinstance(response_data, dict) else {}, "table", response)
            if not isinstance(response_data, dict) or not response_data.get("detailed_description") or not response_data.get("entity_info"):
                response_data = self._coerce_schema(response_data if isinstance(response_data, dict) else {}, "image", response)
